...
ui <- shinyUI(fluidPage(
  fluidRow(
    ...
    column(2, 
           br(),
           br(),
           checkboxInput('histo', label = "histogram"),
           radioButtons('select', "distribution", 
                        choices = c("original", "Gaussian mixture", "K-means clustering"),
                        selected="original"),
           checkboxGroupInput('display', label = "display", 
                              choices = c("components (pdf)", "mixture (pdf)","data"),
                              selected = c("components (pdf)", "mixture (pdf)","data"))
    )
  )
))
...
server <- shinyServer(function(input, output) {
  ...
  r <- reactive({
    ...
    f <- f1 + f2
    pdf1 <- data.frame(x,f)
    pdf2 <- data.frame(x=c(x,x),ori=c(f1, f2),g=as.factor(c(rep(1,nx),rep(2,nx))))
    n1 <- rbinom(1,n,p1)
    n2 <- n-n1
    y <- c(rnorm(n1,mu1,sigma1),rnorm(n2,mu2,sigma2))
    d <- data.frame(g=as.factor(c(rep(1,n1),rep(2,n2))),y=y)
    ...
    res <- round(res,3)
    res$missclass. = c(0, paste0(mean(d$g.em!=d$g)*100,"%"), paste0(mean(d$g.km!=d$g)*100,"%")) 
    row.names(res) <- c("original", "Gaussian mixture", "K-means clustering")
  ...
  output$plot1 <- renderPlot({
    ...
    if (input$select=="original") {
      ...
      if ("components (pdf)" %in% input$display)
        pl <- pl + geom_line(data=pdf2, aes(x, ori, color=g), size=sl)  
      if ("data" %in% input$display)
        pl <- pl + geom_point(data=d, aes(x=y,y=z, colour=g))
    } else if (input$select=="Gaussian mixture") {
      if ("mixture (pdf)" %in% input$display)
        pl <- pl + geom_line(data=pdf1, aes(x, f.em), color="black",  size=sl) 
      if ("components (pdf)" %in% input$display)
        pl <- pl + geom_line(data=pdf2, aes(x, em, color=g),  size=sl)  
      if ("data" %in% input$display) {
      ...
      if ("components (pdf)" %in% input$display)
        pl <- pl + geom_line(data=pdf2, aes(x, km, color=g),  size=sl)  
      if ("data" %in% input$display) 
